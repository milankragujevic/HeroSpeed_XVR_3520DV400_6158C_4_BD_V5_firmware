...
if (ENVIRONMENT_IS_NODE) {
    ...
    if (ENVIRONMENT_IS_WEB) {
        if (document.currentScript) scriptDirectory = document.currentScript.src
    } else {
    ...
    Module["readAsync"] = function readAsync(url, onload, onerror) {
        ...
        xhr.onload = function xhr_onload() {
            if (xhr.status == 200 || xhr.status == 0 && xhr.response) return onload(xhr.response);
            onerror()
        };
        xhr.onerror = onerror;
        xhr.send(null)
...
function integrateWasmJS() {
    ...
    var wasmBinaryFile = "libde265.wasm";
    ...
    Module["reallocBuffer"] = (function (size) {
        return finalMethod === "asmjs" ? asmjsReallocBuffer(size) : wasmReallocBuffer(size);
    });
    var finalMethod = "";
    ...
    node_ops: {
        ...
        rename: (function (old_node, new_dir, new_name) {
            if (FS.isDir(old_node.mode)) {
                ...
                if (new_node) {
                    for (var i in new_node.contents) throw new FS.ErrnoError(ERRNO_CODES.ENOTEMPTY)
                }
            }
...
var FS = {
    ...
    lookupPath: (function (path, opts) {
        ...
        for (var i = 0; i < parts.length; i++) {
            ...
            if (FS.isMountpoint(current) && (!islast || islast && opts.follow_mount)) current = current.mounted.root
    ...
    mayOpen: (function (node, flags) {
        ...
        } else if (FS.isDir(node.mode) && FS.flagsToPermissionString(flags) !== "r" || flags & 512) {
    ...
    read: (function (stream, buffer, offset, length, position) {
        if (length < 0 || position < 0) throw new FS.ErrnoError(ERRNO_CODES.EINVAL)
        if (FS.isClosed(stream)) throw new FS.ErrnoError(ERRNO_CODES.EBADF)
    ...
    createLazyFile: (function (parent, name, url, canRead, canWrite) {
        ...
        LazyUint8Array.prototype.cacheLength = function LazyUint8Array_cacheLength() {
            ...
            var doXHR = (function (from, to) {
                ...
                xhr.send(null);
                if (!(xhr.status >= 200 && xhr.status < 300 || xhr.status === 304)) throw new Error("Couldn't load " + url + ". Status: " + xhr.status);
                return xhr.response !== undefined ? new Uint8Array(xhr.response || []) : intArrayFromString(xhr.responseText || "", true);
            });
            var lazyArray = this;
...
var SYSCALLS = {
    ...
    doStat: (function (func, path, buf) {
        try {
            ...
            if (e && e.node && PATH.normalize(path) !== PATH.normalize(FS.getPath(e.node))) return -ERRNO_CODES.ENOTDIR
            throw e
        }
...
function _strftime(s, maxsize, format, tm) {
    ...
    function compareByDay(date1, date2) {
        ...
        var compare;
        if ((compare = sgn(date1.getFullYear() - date2.getFullYear())) === 0 && (compare = sgn(date1.getMonth() - date2.getMonth())) === 0)
    ...
    function getWeekBasedYear(date) {
        ...
        if (compareByDay(firstWeekStartThisYear, thisDate) <= 0) {
            return compareByDay(firstWeekStartNextYear, thisDate) <= 0 ? thisDate.getFullYear() + 1 : thisDate.getFullYear();
        } else {
...
Image.prototype.get_width = (function () {
    if (this.width === null) this.width = libde265.de265_get_image_width(this.img, 0)
    return this.width
});
Image.prototype.get_height = (function () {
    if (this.height === null) this.height = libde265.de265_get_image_height(this.img, 0)
    return this.height
});
Image.prototype.display = (function (imageData, callback) {
    ...
    _free(stride);
    // this.decoder.convert_yuv2rgb(chroma, y, u, v, w, h, stridey, strideu, stridev, bppy, bppu, bppv)//*旧代码2020.11.23*
    this.decoder.convert_yuvData(y, u, v, w, h, stridey, strideu, stridev);//*新渲染方式[转化为yuv格式2020.11.23*
    // this.decoder.convert_yuvData(y, u, v, w, h, stridey, strideu, stridev)
});
...
Decoder.prototype.consumeData = (function (vdata) {
    var that = this;
    if (!this.started && this.isPausing) return;

    if (!vdata) {
        var d = this.toBeDecodeQueue.shift();
        if (d) {
            var err = decoder.push_data(d);
...
Decoder.prototype.convert_yuvData = (function (y, u, v, w, h, stridey, strideu, stridev) {
    ...
    var d = { "w": w, "h": h, 'yuvData': yuvData, "id": this.canvasId };
...
onmessage = (function (e) {
    ...
    if (decoder) {
        if (!e.data.cmd) {
            decoder.toBeDecodeQueue.push(data);
            decoder.consumeData();
        } else {
            decoder.canvasId = e.data.id;
            decoder.consumeData(data);
        }
    }
})